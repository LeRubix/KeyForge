const WORD_LIST = [
  ...
  'essence', 'establish', 'estate', 'eternal', 'ethics', 'evidence', 'evil', 'evoke', 'evolve', 'exact',
  ...
  'pitch', 'pizza', 'place', 'planet', 'plastic', 'plate', 'play', 'playground', 'pleasant', 'please',
  ...
  'ripe', 'rise', 'risk', 'ritual', 'rival', 'river', 'road', 'roast', 'robot', 'robust',
  'rocket', 'romance', 'roof', 'rookie', 'room', 'rose', 'rotate', 'rough', 'round', 'route',
  'royal', 'rubber', 'rude', 'rug', 'rule', 'run', 'runway', 'rural', 'sad', 'saddle',
  'sadness', 'safe', 'sail', 'salad', 'salmon', 'salon', 'salt', 'same', 'sample', 'sand',
  ...
  'weapon', 'weary', 'weather', 'weave', 'web', 'wedding', 'weekend', 'weird', 'welcome', 'west',
...
export function generateRecoveryPhrase(): string[] {
  ...
  for (let i = 0; i < 15; i++) {
    const index = (array[i] + (array[i + 1] << 8)) % WORD_LIST.length;
...
export async function recoveryPhraseToKey(phrase: string[]): Promise<string> {
  const phraseString = phrase.join(' ');
  const encoder = new TextEncoder();
  const data = encoder.encode(phraseString);
  const salt = encoder.encode('keyforge-recovery-salt-v1');
  ...
  const keyArray = Array.from(new Uint8Array(keyBits));
